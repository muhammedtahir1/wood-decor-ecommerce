...
const OrdersPagination = ({ initialOrders }: { initialOrders: ExtendedOrder[] }) => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const apiProducts = await getOrders({ take: NUMBER_OF_PRODUCTS_TO_FETCH, skip: 0 });
  ...
  const loadMoreProducts = async () => {
    ...
    setLoading(true);
    try {
      const apiProducts = await getOrders({ take: NUMBER_OF_PRODUCTS_TO_FETCH, skip: offset });
    ...
    <div><Table >
      ...
      {!loading && hasMore && (
        <div className='flex mx-auto'>
          ...
          <Button onClick={loadMoreProducts} className="mb-10 mt-5 md:mb-20 mx-auto">Load More...</Button>
        </div>
      )}
    </div>
  )
