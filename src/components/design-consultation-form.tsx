...
export default function DesignConsultationForm() {
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault()
    const whatsappNumber = "6362964875"
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold">Get a free design consultation</DialogTitle>
          <Button
            variant="ghost"
            size="icon"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              onCheckedChange={(checked) => setFormData(prev => ({ ...prev, whatsappUpdates: checked as boolean }))}
            />
            <Label htmlFor="whatsappUpdates" className="text-sm">
              Yes, send me updates via WhatsApp.
            </Label>
          </div>
          <Button type="submit" className="w-full bg-red-700 hover:bg-red-800">Book a Free Consultation</Button>
